...
class AchievementsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val achievements = getAchievements(watchlistRepository)
        ...
        val completedCount = achievements.count { it.currentProgress >= it.maxProgress }
        watchlistRepository.setSeenAchievementsCount(completedCount)
    }
    ...
    companion object {
        fun getCompletedCount(repo: WatchlistRepository): Int {
            return getAchievements(repo).count { it.currentProgress >= it.maxProgress }
        }

        fun getAchievements(repo: WatchlistRepository): List<Achievement> {
            val stats = repo.getStats()
            val diary = repo.getDiary()
            val vitrina = repo.getVitrinaMovies()
            val lists = repo.getUserLists()
            ...
            return listOf(
                ...
                Achievement("Identidad Visual", "Cambia tu foto de perfil", if (repo.getProfileImageUri() != null) 1 else 0, 1)
            )
        }
    }
}
